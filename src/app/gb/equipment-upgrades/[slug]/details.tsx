...
export const getEquipmentUpgrade = async (slug: string) => {
  return await fetch(`/gb/api/equipment/upgrades/${slug}`).then(response => {
...
export function EquipmentUpgradeDetails({ slug } : { slug: string }) {
  ...
  const {data, isFetching } = useQuery({
    queryKey: ['gb-equipment-upgrade-details', slug],
    queryFn: () => getEquipmentUpgrade(slug),
  });
