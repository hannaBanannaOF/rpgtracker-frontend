...
export function EquipmentUpgradesList() {
  ...
  const {data, isFetching } = useQuery({
    queryKey: [ReactQueryKeys.Ghostbusters.equipmentUpgradesList],
    queryFn: async () => {
      return await client.get('/gb/api/equipment/upgrades') as ListItem[];
    },
  });
  ...
  return <>
    ...
    {menu && menu.checkPermission('ghostbusters', 'create') && <Group justify={"end"}>
      ...
      <Modal opened={opened} onClose={close} centered title={t("form.title")} size={"lg"}>
        <EquipmentUpgradeForm onSubmit={close}/>
      </Modal>
    </Group>}
    {data && data.length > 0 &&
      <Box pos={"relative"}>
